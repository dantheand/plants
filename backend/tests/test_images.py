...
class TestImageRead:
    def test_get_image_link(self, client, mock_db, image_record):
        image = image_record
    ...
    def test_get_all_image_links_for_plant(self, client, mock_db, default_user_plant):
    ...
    def test_get_images_for_other_users_plant_ok(self, mock_db, client, default_user_plant):
        plant = default_user_plant
        for _ in range(10):
            create_and_insert_image_record(mock_db, plant_id=plant.plant_id)
...
class TestImageUpload:
    def test_upload_image_for_plant(self, client, mock_db, fake_s3, default_user_plant):
        plant = default_user_plant
    ...
    def test_thumbnail_creation(self, client, mock_db, fake_s3, default_user_plant):
        plant = default_user_plant
    ...
    def test_upload_image_w_timestamp(self, mock_db, client, fake_s3, default_user_plant):
        plant = default_user_plant
...
class TestImageDelete:
    def test_delete_image_from_db(self, mock_db, client, fake_s3, plant_with_image_record):
    ...
    def test_delete_image_fails_if_not_owner(self, mock_db, client, plant_with_image_record):
        _, image = plant_with_image_record

        response = client(OTHER_TEST_USER).delete(f"/images/{image.image_id}")
...
class TestImageUpdate:
    def test_update_image_timestamp(self, mock_db, client, plant_with_image_record):
        plant, image = plant_with_image_record
