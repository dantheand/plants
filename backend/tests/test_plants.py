...
class TestPlantRead:
    def test_get_your_plant_list(self, client_mock_session, mock_db):
    ...
    def test_read_missing_plant(self, client_mock_session, mock_db):
        plant_user_id = DEFAULT_TEST_USER.google_id
        plant_id = uuid.uuid4()
        response = client_mock_session().get(f"{PLANT_ROUTE}/{plant_user_id}/{plant_id}")
        assert response.status_code == 404

    def test_read_plant_by_human_id(self, client_mock_session, default_user_plant):
        plant = default_user_plant
...
class TestPlantCreate:
    def test_create(self, client_mock_session, mock_db):
        test_client = client_mock_session(DEFAULT_TEST_USER)
    ...
    def test_create_with_duplicate_plant_id(self, client_mock_session, mock_db, default_user_plant):
        test_client = client_mock_session(DEFAULT_TEST_USER)
    ...
    def test_create_missing_required_fields(self, client_mock_session):
        test_client = client_mock_session(DEFAULT_TEST_USER)
        new_plant = plant_record_factory(human_name="New Plant")
...
class TestPlantUpdate:
    def test_update(self, client_mock_session, mock_db, default_user_plant):
        plant = default_user_plant
    ...
    def test_update_other_users_plant_fails(self, client_mock_session, mock_db, default_user_plant):
        plant = default_user_plant

        # Another user who is not the owner of the plant
        other_user_client = client_mock_session(OTHER_TEST_USER)
    ...
    def test_update_missing_plant_fails(self, client_mock_session, mock_db):
        plant_id = uuid.uuid4()
        updated_plant = PlantBase(**plant_record_factory().model_dump())
        test_client = client_mock_session()
        response = test_client.patch(f"{PLANT_ROUTE}/{plant_id}", json=updated_plant.dynamodb_dump())
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_update_cant_change_human_id(self, mock_db, client_mock_session, default_user_plant):
        plant = default_user_plant
...
class TestPlantDelete:
    def test_delete(self, client_mock_session, mock_db, default_user_plant):
        plant = default_user_plant
    ...
    def test_delete_other_users_plant_fails(self, client_mock_session, mock_db, default_user_plant):
        plant = default_user_plant
    ...
    def test_delete_missing_plant_fails(self, client_mock_session, mock_db):
        plant_id = uuid.uuid4()
        test_client = client_mock_session()
        response = test_client.delete(f"{PLANT_ROUTE}/{plant_id}")
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_delete_plant_deletes_images_in_db(self, client_mock_session, mock_db, fake_s3, plant_with_image_record):
    ...
    def test_delete_plant_deletes_images_in_s3(self, client_mock_session, mock_db, fake_s3, plant_with_image_in_s3):
